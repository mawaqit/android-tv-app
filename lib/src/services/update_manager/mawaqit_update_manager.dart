...
class MawaqitUpdateManager extends UpdateManager
    with UpdateManagerDialogsMixin {
  @override
  Future<String?> checkForUpdate() async {
    final currentVersion = '1.6.0' ??
        await PackageInfo.fromPlatform().then(
          (value) => value.version,
        );
    final newVersion = '1.6.4' ?? await Api.getLatestVersion();

    if (compareTwoVersion(oldVersion: currentVersion, newVersion: newVersion))
      return newVersion;
