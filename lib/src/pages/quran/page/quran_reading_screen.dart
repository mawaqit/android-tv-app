...
class _QuranReadingScreenState extends ConsumerState<QuranReadingScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        _initializeQuranDownload();
      }
    });
  }

  Future<void> _initializeQuranDownload() async {
    if (!mounted) return;
    ...
    try {
      ...
      if (mounted) {
        log('[initialize_quran_download]: $e');
      }
    }
  }
                            ...
                            if (rightPageIndex < quranReadingState.svgs.length)
                              Positioned(
                                ...
                                child: _buildSvgPicture(
                                  quranReadingState.svgs[rightPageIndex %
                        ...
                        child: Container(
                          ...
                          child: Text(
                            S.of(context).quranReadingPage(
                                  quranReadingState.currentPage +
                                      1, // Right page (now on the left)
                                  quranReadingState.currentPage +
                                      2, // Left page (now on the right)
                                  quranReadingState.totalPages,
                                ),
                            style: TextStyle(
                              color: Colors.white,
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Container(
            width: double.maxFinite,
            height:
                60.h, // Set a fixed height or use MediaQuery to calculate it
